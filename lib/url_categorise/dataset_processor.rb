  ...
  class DatasetProcessor
    ...
    def process_kaggle_dataset(dataset_owner, dataset_name, options = {})
      ...
      unless response.success?
        raise Error, "Failed to download Kaggle dataset: #{response.message}"
      end
    ...
    def process_csv_dataset(url, options = {})
      ...
      unless response.success?
        raise Error, "Failed to download CSV dataset: #{response.message}"
      end
    ...
    def handle_extracted_dataset(extracted_dir, options)
      ...
      unless csv_files.empty?
        return parse_csv_files_to_hash(csv_files)
      end
    ...
    def parse_csv_content(content, options = {})
      data = []
    ...
    def detect_url_columns(sample_row)
      url_indicators = ['url', 'domain', 'website', 'site', 'link', 'address']
    ...
    def detect_category_columns(sample_row)
      category_indicators = ['category', 'class', 'type', 'classification', 'label']
      sample_row.keys.select do |key|
        key_lower = key.to_s.downcase
    ...
    def extract_domain(url)
      ...
      url = "http://#{url}" unless url.match?(/\A\w+:\/\//)
      ...
      cleaned = url.gsub(/\A\w+:\/\//, '').gsub(/\/.*\z/, '').downcase
